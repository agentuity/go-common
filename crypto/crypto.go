...
// The format of each chunk is: [4-byte chunk size][nonce][encrypted data][tag]
func DecryptStream(reader io.Reader, writer io.WriteCloser, key string) error {
	...
	for {
		...
		// Convert 4 bytes to uint32 (chunk size)
		chunkSize := uint32(sizeBuffer[0]) | uint32(sizeBuffer[1])<<8 |
			uint32(sizeBuffer[2])<<16 | uint32(sizeBuffer[3])<<24
		...
		// Read encrypted chunk (includes tag)
