...
func main() {
	services := []string{"aether", "aigateway", "auth", "catalyst", "otel", "registry", "streams", "ion"}
...
`
	...
	seenRegions := make(map[network.Region]string)
	...
	for _, name := range names {
		...
		// Skip if we've already generated constants with this name or region value
		if seenConstNames[regionName] {
			continue
		}
		if _, seen := seenRegions[region]; seen {
			continue
		}

		seenRegions[region] = name
		seenConstNames[regionName] = true
	...
	// Get all region values and sort them
	var allRegionValues []network.Region
	for region := range seenRegions {
		if region != network.RegionGlobal {
			allRegionValues = append(allRegionValues, region)
		}
	}
	sort.Slice(allRegionValues, func(i, j int) bool {
		return allRegionValues[i] < allRegionValues[j]
	})
	...
	for _, region := range allRegionValues {
		name := seenRegions[region]
